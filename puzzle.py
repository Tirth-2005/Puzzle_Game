...
def ShowStarInterface(screen, width, height):
    screen.fill(cfg.BGCOLOR)
    ...
    content2 = cfont.render("H- 6x6, M- 4x4, L- 3x3", True, cfg.BLUE)
...
def main():
    ...
    game_img_used = pygame.transform.scale(game_img_used, cfg.S_SIZE)
    game_img_used_rect = game_img_used.get_rect()

    screen = pygame.display.set_mode(cfg.S_SIZE)
    pygame.display.set_caption('Pokemon')

    size = ShowStarInterface(screen, game_img_used_rect.width, game_img_used_rect.height)
    assert isinstance(size, int)
